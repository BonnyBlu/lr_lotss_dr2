...
def generate_random_catalogue(n, ra1=0.0, ra2=360.0, dec1=-90, dec2=90.0):
...
def area_ra_dec(ra_down, ra_up, dec_down, dec_up):
    ...
    """
    return (
        (np.deg2rad(ra_up) - np.deg2rad(ra_down))
...
class Field(object):
    ...
    def __init__(self, ra_down, ra_up, dec_down, dec_up):
        ...
        self.dec_up = dec_up
        self.area = area_ra_dec(self.ra_down, self.ra_up, self.dec_down, self.dec_up)
    ...
    def random_catalogue(self, n):
        ...
        return generate_random_catalogue(
            n, ra1=self.ra_down, ra2=self.ra_up, dec1=self.dec_down, dec2=self.dec_up
        )
...
class Q_0(object):
    ...
    def __init__(self, coords_small, coords_big, field, radius=5.0):
    ...
    def __call__(self, radius=None):
        ...
        return 1.0 - float(nomatch_small) / float(nomatch_random)
...
def get_sigma(
    ...
    * sigma: Combined covariance matrix
    ...
    # Additional error
    sigma_additonal_error = np.array(
        [[additional_error ** 2, 0], [0, additional_error ** 2]]
    )
    sigma = sigma_radio + sigma_optical + sigma_additonal_error
    return sigma


def get_sigma_all_old(
    maj_error,
    min_error,
    pos_angle,
    radio_ra,
    ...
    * sigma_min: Error in the minor axis direction
    ...
    maj_squared = (
        (majerr * np.cos(sigma)) ** 2
        + (opt_ra_err * np.cos(phi)) ** 2
        + additonal_error ** 2 / 2.0
    )
    min_squared = (
        (minerr * np.sin(sigma)) ** 2
        + (opt_dec_err * np.sin(phi)) ** 2
        + additonal_error ** 2 / 2.0
    )
    return (
        np.sqrt(maj_squared + min_squared),
        np.sqrt(maj_squared),
        np.sqrt(min_squared),
    )
...
def estimate_q_m_kde(
    magnitude, bin_centre, n_m, coords_small, coords_big, radius=5, bandwidth=0.2
):
    """Compute q(m)
    Estimation of the distribution of real matched sources with respect 
...
class SingleMLEstimatorOld(object):
    ...
    def __call__(self, m, r, sigma, sigma_maj, sigma_min):
        """Get the likelihood ratio"""
        return (
            fr_u_old(r, sigma, sigma_maj, sigma_min) * self.get_qm(m) / self.get_nm(m)
        )
...
class MultiMLEstimatorOld(object):
    ...
    def __call__(self, m, r, sigma, sigma_maj, sigma_min, k):
        """Get the likelihood ratio"""
        return (
            fr_u_old(r, sigma, sigma_maj, sigma_min)
            * self.get_qm_vect(m, k)
            / self.get_nm_vect(m, k)
        )
...
def q0_min_numbers(q_0_list, numbers_combined_bins):
    ...
    """
    q_0 = np.array(q_0_list)
    thresholds = 1.0 / numbers_combined_bins
...
def parallel_process(
    ...
    if n_jobs == 1:
        return front + [
            ...
            for a in tqdm_f(array[front_num:])
        ]
