...
##SBATCH --array=1-54                    ##  Number of jobs
...
Help()
{
    ...
    This script the LoTSS_params.py script. It takes a single input file which 
    contains the location of the radio, Gaussian and optical catalogues, and outputs
    ...
    1.  The script will look for /data and /config in the working directory. It will 
...
# This section will call the inputs from the .yaml file.

eval "$(${WORKING_DIR}/config/call_yaml.py ${WORKING_DIR}/config/inputs.yml lr_inputs thres_calc)"
...
## Run the params and threshold script
if [ "$THRES_CALC" = "True" ]; then
    ...
    echo "The threshold and parameters calculation is not requested. Edit the config.yml file if these calculations are required. Exiting."
fi
